...
class Settings(BaseSettings):
    ...
    def database_url(self) -> str:
        """生成数据库连接 URL，自动修复驱动名称"""
        if self.DATABASE_URL:
            url = self.DATABASE_URL
            # 将异步驱动替换为同步 pymysql（Streamlit 使用同步模式）
            for driver in ("mysql+asyncmy", "mysql+aiomysql"):
                if driver in url:
                    url = url.replace(driver, "mysql+pymysql")
                    break
            else:
                if "mysql+pymysql" not in url and "mysql://" not in url:
                    url = url.replace("mysql://", "mysql+pymysql://", 1) if "mysql://" in url else url
            return url
        # 从独立配置项构建连接 URL
        return (
    ...
    # ChromaDB 向量数据库配置
    # ============================================================
    CHROMA_PERSIST_DIR: str = "./chroma_data"  # 持久化存储目录
