...
def create_resume(output_path: Path):
    doc = Document()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="生成优化版 Word 简历")
    parser.add_argument(
        "output",
        nargs="?",
        type=Path,
        default=Path("赵冠杰_简历_优化版.docx"),
        help="输出 .docx 路径",
    )
    create_resume(parser.parse_args().output)
